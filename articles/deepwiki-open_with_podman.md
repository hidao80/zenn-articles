...
- **前提知識**: 
  - WSL[^1]、Podman[^2]、DeepWiki[^3]、Ollama[^4]、Git、GitHubがどのようなものであるかと使用方法
  - Dockerの利用経験：初級程度
...
- **金銭的コスト削減**: OpenAI APIなどの有料サービスを使わずに済む
...
:::details PodmanとPodman-Desktop、Composeのインストール
Podmanには標準で`podman compose`というオプションがありますが、これはデフォルトでは外部の `docker-compose` コマンドを優先して呼び出すため、`Podman-Desktop`とその中の拡張機能`Compose`を利用します。

```sh:Podmanのインストール
winget install --id Redhat.Podman -h
winget install --id Redhat.Podman-Desktop -h
```

`Podman-Desktop`を起動するとPodman用の仮想マシンをWSL上に作成するダイアログが表示されますので、画面の指示にしたがって仮想マシンを作成し`compose`をインストールします。

**仮想マシンを作成できなかったとき**

下記コマンドで仮想マシン`podman-machine-default`を作成できます。

```sh:podman用仮想マシンの作成
podman machine init
```

その後、`Podman-Desktop`のDashboardウインドウ→Settings→Resourcesから`Compose`をインストールしてください。

![図:Podman-DesktopからComposeをインストールする](/images/deepwiki-open_with_podman/podman-desktop_1.png)
*図:Podman-DesktopからComposeをインストールする*
:::

:::details deepwiki-openの取得
Windowsの任意の作業フォルダーで`deepwiki-open`のリポジトリをcloneします。

```cmd:deepwiki-openのclone
...
Docker Desktopが動いていれば`host.docker.internal`が利用でき、Podman-Desktopが動いていれば`host.containers.internal`をドメインとしてOllamaサーバーへのアクセスが可能です。  
今回はPodman-Desktopを利用するため、少なくとも`host.containers.internal:host-gateway`が必要です。`host.docker.internal:host-gateway`も併記しておくとDocker Desktop環境でも動作するため汎用性が高くなります。
...
3. ホーム画面上部のリポジトリパス入力テキストボックスに`docker-compose.yml`で指定したボリューム先のパス`/app/repo`を指定して「Wikiを検索」ボタンをクリックします。詳細設定ダイアログが開きます。
...
Windows側でOllamaが起動していますか？Exeから起動しておくか、コマンドラインから`ollama serve`でOllamaをサーバーとして起動しておく必要があります。
...
「PodmanとPodman-Desktop、Composeのインストール」開閉パネルの中でインストールしているPodman-Desktop用の拡張機能`Compose`をPodman-Desktopのダッシュボードウインドウからインストールしてください。
...
ほぼこのままの形でOpenAI APIやGoogle API、Claude APIに対応できます。ただし、その場合はAPIコストが発生してしまいます。  
